...
roster при этом уже
проверены отдельным прогоном на SQLite 3.45: 61 сценарий с машинными
...
ненулевой код, пока эти 23 не получат исполняемые свидетели; зелёная
trigger-parity не выдаётся за полную parity схемы. Для обязательного гейта
известный долг закреплён точным allow-list в `debt-baseline.tsv`: варианты
`--coverage-baseline` и `--schema-sync-baseline` принимают тот же набор или его
подмножество и падают на любом новом ID даже при прежнем общем числе.
...
решение. Читать обязательно: там тридцать пять уточнений с обоснованием,
включая одну необязательную возможность и один явный пересмотр Q56.
...
Реестр нормативен для новых таблиц, но его ретроспективная полнота пока не
доказана: из 38 durable-таблиц здесь классифицированы 17, ещё 21 требует
отдельного lifecycle-аудита (`author_revision`, `campaign_transition`,
`finding`, `blocker`, task/artifact-таблицы и другие). Это не означает, что все
21 должны стать append-only: для каждой сначала нужны owner, разрешённые
операции, recovery и контрпримеры. Классифицировать их по одной «заодно» с
чужой правкой запрещено — это отдельная граница анализа, а не косметика.
...
**`dedup_key` — диагностический признак, не механизм группировки или identity.**
Состав ключа сохраняется прежним: canonical
`severity + title + body + file + start + end`. Это дешёвый объяснимый сигнал
для отчёта `possible_duplicate` Q54 и обратимая телеметрия на случай, если после
реальных прогонов понадобится снова исследовать дословные повторы.

По решению Q56 рантайм **никогда не склеивает observations по этому ключу**.
Каждая immutable-строка передаётся reconciler'у отдельной позицией со своим
public ID и своим `evidence`; `dedup_key` не входит в prompt, alias map или
validation context. Даже одинаковый ключ не доказывает одну проблему: ключ не
содержит `evidence`, а значит автоматическое слияние могло бы убрать отдельное
доказательство или целую проблему. Semantic group создаёт только reconciler,
явно перечисляя raw observation IDs.

Приёмочный контрпример обязателен: две строки с одинаковыми шестью полями и
разным `evidence` получают одинаковый диагностический key, но остаются двумя raw
входами; оба ID и оба evidence доходят до reconciliation и покрываются его
исчерпывающим разбиением. Никакого теста «совпадение автоматически склеилось»
больше нет.
...
| `first_seen high(10)` и `override→medium(10)` в том же событии | 10 | До override нет более ранней severity текущего периода; `medium` становится начальным значением накопителя |
| `verified_fixed(20)`, затем `reopening low(30)` и `override→medium(30)` в том же событии | 30 | На границе нового периода действует то же правило: observation события отсечён, начальное значение — `medium` |
| `finding(10)`, `override→high(20)`, `first_seen low(30)` | 10 | Схема не связывает события finding и first_seen; до override нет вычислимой severity, поэтому `high` становится начальным значением |
| `verified_fixed(20)`, затем `override(30)` без reopening | NULL | DDL-valid строка остаётся audit-фактом вне открытого периода и не участвует в nullable projection |

`finding_last_override` включает границу периода через `>=`, а observations после
override — через строгое `>`. Если до override в текущем периоде нет более
ранней вычислимой severity, он задаёт начальное значение: проверить
`old_severity` не с чем. Это верно не только в самом открывающем событии — DDL
допускает, что `finding.event_id`, override и первый `first_seen` link идут
тремя последовательными событиями. Когда предыдущее значение существует,
override обязан с ним согласоваться. Строка вне открытого периода не создаёт
отсутствующий период и игнорируется view; writer вправе запретить её своим
предусловием, но pure-oracle не приписывает SQLite отсутствующий constraint.
...
Проектирование и последующее ревью таск-планов дали тридцать пять уточнений:
тридцать три обязательных изменения, одну возможность (пункт 5) и один явный
пересмотр решения (пункт 30). Пункты 1–29 и 31–35 не отменяют принятых решений —
без них часть решений нереализуема. Пункт 30 фиксирует ответ владельца Q56 B:
прежний automatic pre-grouping отменён, диагностическая телеметрия сохранена.
...
**30. `dedup_key` не задаёт входную единицу reconciliation.** В первой
редакции одинаковый ключ превращал несколько raw observations в одну
`ReconciliationUnit`: prompt показывал representative и aliases, а validator
затем раскрывал их обратно. Решение Q56 B отменяет этот слой. Причина не только
в редкости дословных совпадений: fingerprint не содержит `evidence`, поэтому
две строки с разными доказательствами могли схлопнуться до классификации.

Колонка и индекс остаются как обратимая диагностическая телеметрия для Q54, но
ключ не входит в prompt, validation context, группировку или выдачу identity.
Reconciler получает плоский список raw observations, видит каждый ID/evidence и
...
**34. Cut-off реализации различает две роли Git SHA.** После закрытия C-19
planning baseline пакета и implementation parent задачи стали разными
границами, но прежний `handoff.cutoff.v1` имел только неименованный список
`referenced_shas`. При передаче между сессиями два существующих SHA нельзя было
однозначно восстановить без разбора свободного текста.

DDL не меняется. В `agent-contracts.md` §9 добавлен обязательный nullable
`implementation_scope`: для implementation handoff он содержит оба именованных
full SHA, точно совпадает с immutable `CutoffContext`, и оба значения входят в
`referenced_shas`; для остальных видов работы значение равно `null`. Это
уточняет решение `decision.md` §9.3 и не отдаёт агенту authority выбирать
planning baseline или левую границу реализации.

**35. Override без предшествующей severity имеет определённую семантику.** DDL
разрешает `severity_override.event_id` совпасть с событием создания finding или
`reopening`, а также оказаться между `finding.event_id` и событием его первого
`first_seen` link. `finding_last_override` включает строку через условие
`event_id >= period_start_event_id`, а observations после cutoff — только через
строгое `link.event_id > override.event_id`. Поэтому при отсутствии более
ранней вычислимой severity override становится начальным значением периода;
`old_severity` сверять не с чем. Если предыдущее значение есть, совпадение
обязательно. Override вне открытого периода остаётся append-only audit-фактом и
не участвует в nullable projection. Writer может не создавать эти формы, но
pure-oracle не отвергает состояние, которое принимает и однозначно читает
нормативная схема.
