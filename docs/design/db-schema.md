...
override — через строгое `>`. Поэтому override в самом открывающем событии
задаёт начальное значение периода: проверить его `old_severity` по более ранней
severity этого периода невозможно. Override строго внутри периода обязан
сверяться с накопителем перед своим событием. Строка вне открытого периода не
создаёт отсутствующий период и игнорируется view; writer вправе запретить её
своим предусловием, но pure-oracle не приписывает SQLite отсутствующий
constraint.
...
**35. Override в открывающем событии имеет определённый порядок.** DDL
разрешает `severity_override.event_id` совпасть с событием создания finding или
`reopening`. `finding_last_override` включает такую строку через условие
`event_id >= period_start_event_id`, а observations после cutoff — только через
строгое `link.event_id > override.event_id`. Поэтому override логически следует
за observation того же
события и становится начальным значением периода; проверить `old_severity` по
более ранней severity внутри этого периода невозможно. Override вне открытого
периода остаётся append-only audit-фактом и не участвует в nullable projection.
Writer может не создавать эти формы, но pure-oracle не отвергает состояние,
которое принимает и однозначно читает нормативная схема.
