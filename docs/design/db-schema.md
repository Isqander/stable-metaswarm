...
он. Доказаны все 68 ограничений группы; манифест умеет держать и признанный
...
выводится из доменного решения (`AskHuman` для `reconcile_failed`,
`AwaitHumanReopen` для `reopen_human_closed`), а не возвращается formatter'ом:
`HumanGateFormatter` в T1.16 — чистая функция, она не читает базу и членство не
выбирает. T1.16 при обработке ответа членство только **читает**.
