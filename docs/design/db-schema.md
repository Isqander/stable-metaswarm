...
`subject_revision`. Остальные
schema-effects того же свода
...
можно передать несколько). Третий прогон, `scope-keys.sql`, — 66 сценариев на
...
`scripts/checks/mutations.tsv` держит для каждого ограничения назначенный шаг,
и `mutation-check.py` требует, чтобы после снятия ограничения покраснел именно
он. Доказаны все 68 мутационных случаев (случай может снимать несколько
перекрывающихся ключей сразу); манифест умеет держать и признанный
долг (`todo: <причина>`), который считается недоказанным и виден в итоге
отдельным числом.
...
выбирает reason. Возвращает он **только presentation** — текст, options, тело
outbox и transport; `reason`, scope, `snapshot_json` и строки
`human_question_observation` пишутся из самого запроса. Поэтому проверка перед
атомарной записью question/outbox/blocker/state/event сузилась до пригодности
presentation: разойтись с решением по причине, scope или snapshot формулировщик
уже не может — он их не производит.
