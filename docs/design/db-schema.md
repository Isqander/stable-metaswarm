...
давал ложный зелёный результат. Реализатор либо придумал бы несовместимые
правила, либо принял двусмысленный результат.
...
`steps`/`rationale`. Cut-off получил структурные поля, а SHA-256 считает сервис
