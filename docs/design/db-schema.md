...
проверок verification policy (`path` и `target`). Реализатор либо придумал бы
несовместимые правила, либо принял двусмысленный результат.
...
модели запрещают extra/coercion. Cut-off получил структурные поля, а SHA-256
считает сервис по canonical payload после валидации. Все внешние факты
