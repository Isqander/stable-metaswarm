...
`human`-закрытого, роль `reconciler`. Завершённый reconciliation заодно
возвращает исход первичного `discovery`-круга: `clean` без открытых findings
либо `needs_revision` при их наличии; через `decide_after_check` этот круг не
идёт.
