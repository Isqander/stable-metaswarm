...
| **M1** | Чистая логика findings доказана без процессов и SQLite | Доменные переходы, caps, reconciliation/severity и strict-контракты из строк M1 матрицы ниже проходят unit-тестами |
...
| **M3** | Флоу «ревью коммита» проходит на фейковых CLI | Все строки матрицы с полной приёмкой M3 зелёные; Telegram и task graph сюда не приписываются |
...
системе появится хоть один запущенный процесс, и проверяет чистую часть
протокола. Он не обещает process-, recovery-, transport- и graph-сценарии §14.
...
### 2.1. Матрица приёмочных сценариев §14

ID закреплены непосредственно в `decision.md` §14. «Раннее доказательство» —
часть составного сценария, уже проверенная на более раннем рубеже; строка не
считается полностью принятой до рубежа из последней колонки.

| ID | Сценарий кратко | Раннее доказательство | Полная приёмка / владелец |
|---|---|---|---|
| `S14-01` | Kill на четырёх границах | M1b: атомарные DB-переходы | **M2**, T1.10a/T1.10b |
| `S14-02` | Duplicate Telegram update и outbox delivery | M2: повтор outbox delivery | **T1.15 после M3**: Telegram-часть; не блокирует M4 |
| `S14-03` | Старый, чужой или закрытый human answer | M1b: scope и immutable membership | **M2**, T1.16/T1.10b |
| `S14-04` | Blocker без вопроса и наоборот | M1b: review-gate транзакции | **M2**, recovery audit T1.10b |
| `S14-05` | Все исходы vendor CLI | M0: байтовые fixtures | **M3**, T1.8/T1.9/T1.18/T1.22a |
| `S14-06` | Мутация checkout ревьюером | M1: contract outcome | **M3**, T1.12–T1.14/T1.22a |
| `S14-07` | Частичный кворум не consensus | M1: чистое правило | **M1b**, roster/completeness seam T1.3/T1.7b |
| `S14-08` | Автор потерял/дублировал finding | — | **M1**, T1.7 contract validation |
| `S14-09` | Отказ автора не принят reviewer | — | **M1**, T1.4/T1.6/T1.7 |
| `S14-10` | Cap, четвёртая проверка, human gate и живой sibling | M1: cap; M1b: gate | **M3**, scheduler/run-state T1.18/T1.22a |
| `S14-11` | Новый finding на последнем круге не сбрасывает cap | M1: причина решения | **M1b**, T1.7b |
| `S14-12` | `new` для открытого finding виден человеку | M1b: blind output сохранён | **M3**, similarity report T1.19/T1.22a |
| `S14-13` | Финальный accepted refusal и human-closed reopen | M1: intents | **M1b**, T1.7b |
| `S14-14` | Различение прежнего и нового baseline failure | M1: policy result | **M2**, T1.14/T1.16/T1.10b |
| `S14-15` | Drift code/config и переимпорт plan graph | M3: code/config drift | **T2.x**: plan-revision reimport; phase-1 часть — T1.17/T1.18/T1.22a |
| `S14-16` | Unknown YAML и canonical executed config | — | **M3**, T1.17/T1.22a |
| `S14-17` | Cycle/dangling task graph без false success | — | **T2.x**, task graph + graph acceptance |
| `S14-18` | Исчерпан hang budget | M1: contract outcome | **M3**, T1.9/T1.18/T1.22a |
| `S14-19` | Исчерпан contract retry budget | M1: typed contract failure | **M3**, T1.7/T1.18/T1.22a |
| `S14-20` | Второй service instance не стартует | — | **P1-B, до M2**, T1.11 |
| `S14-21` | Orphan погашен до новой attempt | — | **P1-B, до M2**, T1.10a/T1.11 |
| `S14-22` | Закрытый ID в `existing_open` | — | **M1**, T1.5/T1.7 |
| `S14-23` | Invalid severity/period link | — | **M1**, T1.6/T1.7 |

M1 состоит только из чистых доказательств, перечисленных в его строках и в
колонке «Раннее доказательство». M1b закрывает DB-швы P1-A. M3 принимает только
достижимую в фазе 1 часть; `S14-02`, graph-часть `S14-15` и `S14-17` сохраняют
собственных поздних владельцев и не становятся фиктивными требованиями T1.22a.
...
4. принятая версия планов фиксируется отдельным **planning baseline**-коммитом,
   полный SHA которого записывается в `docs/task-plans/planning-baselines.tsv`;
...
Planning baseline и левая граница реализации — разные объекты. Первый — один
marker commit принятого пакета планов; его SHA цитируют все реализации пакета и
машинно сверяют с tracked-реестром `docs/task-plans/planning-baselines.tsv`.
Второй — **implementation parent** конкретной задачи: фактический `HEAD` чистого
...
проверяет baseline по tracked-реестру, оба trailers, full SHA, ancestry,
достижимость из `main`, first-parent цепочку и
...
неизвестное значение отвергается FK. Для task graph отдельно доказаны обе
границы semantic ID — внутри импорта и среди активных версий, — а также
self-edge и дубль dependency; проверка цикла остаётся коду той же транзакции.
...
Плоский raw-вход без автоматической склейки, диагностический `dedup_key`, выдача
личности, восемь проверок валидации (`agent-contracts.md` §4), четыре типа связи,
двухшаговое переоткрытие
...
тот же ID требует `reopen_closed`. Плюс: observations с одинаковыми шестью
полями и разным evidence остаются двумя raw-входами, оба целиком доходят до
reconciliation, а объединить их может только явная группа агента;
...
Формирование вопроса, стандартные формулировки вариантов при исчерпании кругов
(ключи задаёт запрос), разбор ответа,
...
Порт возвращает только presentation — текст, формулировки вариантов при
канонических ключах и транспортно-нейтральное тело сообщения. `reason`, scope,
snapshot, членство и **транспорт** пишет вызывающая транзакция: маршрут
внешнего эффекта выбирает конфигурация, а не формулировщик, и в P1-A он всегда
`cli`. Поэтому formatter не может ни сузить scope, ни потерять snapshot, ни
отправить вопрос туда, где отправителя ещё нет. Он не читает БД, не выполняет
I/O и не выбирает reason. Выбор транспорта по конфигурации и второй отправитель
появляются здесь и в T1.15.
...
golden-тесты против ловушек YAML. `max_author_revisions` — целое не меньше 1;
ноль не является скрытым режимом «ревью без правок».

**Готово, когда:** неизвестное поле и `max_author_revisions <= 0` ломают
загрузку; `on`/`yes` не становятся булевыми молча; канонический вид совпадает с
тем, что реально исполнено.

### T1.18 · Планировщик и drift check · 3 д

Событийный цикл, шесть проверок перед запуском, drift check по семи осям с
последствиями каждой. При создании **reviewer и reconciler** attempt до `spawn`
одной TX пишет `reviewer_exposure` по уже resolved provider/model; outcome
попытки на это не влияет и T1.7b exposure задним числом не создаёт. Reconciler
здесь не исключение, а главный случай: по умолчанию он берёт профиль линии с
...
принадлежит либо самому предмету кампании, либо одной из её авторских правок
(для `code`/`task`/`stage` это SHA, для `artifact` — `content_digest`
соответствующей `artifact_revision`, см. `db-schema.md` §5.7). Составные FK
связывают строку допуска с попыткой, но законность значения — предусловие, а не
констрейнт.

Разделение владения по этому пути: **T1.3** — repository-примитивы
(`reserve_reviewer_exposure()` и запись попытки с `subject_revision`);
**T1.18** — сама операция, проверка ревизии и негативные AC; **T1.7b** — только
потребление уже созданных attempt и exposure.

**Дополнительно готово, когда** операция доказана целиком, а не только своими
отказами:

- она применяется к обеим ролям — `reviewer` и `reconciler`;
- `lane0 → reconciler` с той же парой и той же ревизией создаёт **новую**
  попытку, **переиспользует** строку допуска и оставляет `first_attempt_id`
  прежним;
- retry после `contract_error`, `transient`, `hung` или `interrupted` делает то
  же самое: новая попытка, та же строка допуска;
- вторая линия с той же фактической парой получает отказ, и созданная попытка
  откатывается вместе с транзакцией;
- попытка с произвольной ревизией отвергается до `spawn`;
- артефактный `fix_check` принимает digest ревизии из
  `author_revision.artifact_revision_id` и отвергает `artifact_revision` другого
  прогона либо другого `logical_path` — эту связь DDL сейчас не ограничивает, и
  проверка остаётся за операцией (см. C-06b свода P1-A).

Позитивные пункты здесь важнее отрицательных: без них T1.3 будет реализован
...
commit. Перед spawn reconciler в `fix_check` он сначала перечитывает точный
`review_round`: ID обязан существовать, круг — быть open и иметь
`kind='fix_check'`, campaign — оставаться активной. Только затем он запускает те
же authoritative finding-coverage/strict-issued reads T1.3, что и
`T1.7b.apply_reconciliation()`: неизвестный/закрытый круг либо хотя бы один
неответивший owner запрещает создание попытки и spawn. Snapshot/attempt/result
несут один `round_id`.
...
reconciler attempt при неизвестном/закрытом круге либо одном неответившем
owner.
...
`dedup_key` их observations — одна из причин отчёта, но не команда на слияние.
...
Reconciliation prompt получает плоский список raw observations: каждый ID и
каждое evidence присутствуют отдельно, `dedup_key`, representative и aliases в
prompt не попадают.
...
Строки `S14-05`, `S14-06`, `S14-10`, `S14-12`, phase-1 часть `S14-15`,
`S14-16`, `S14-18` и `S14-19` из матрицы §2.1; вместе с уже закрытыми
M1/M1b/M2/P1-B доказательствами они образуют достижимую в фазе 1 часть §14.

**Готово, когда:** перечисленные строки зелёные end-to-end на fake CLI. Это и
есть M3.

`S14-17` и graph-часть `S14-15` переносятся в **T2.x** вместе с самим графом —
прогнать их раньше физически нечем. Telegram-часть `S14-02` принадлежит T1.15
после M3 и не блокирует M4. В первой редакции T1.22 обещала «все сценарии §14»,
и это было невыполнимо по составу фазы.
...
| Фаза 1 | 58.5 + буфер 3 на T1.23 |
| Фаза 2 (блоками) | 29.5–34.5 |
| **До MVP** | **~92–97 рабочих дней** |
...
T1.22, добавило таймауты в T1.14 и буфер на притирку промптов. После уточнения
границ P1-A оценки расширившихся T1.3 и T1.7b подняты с 2 до 3 дней каждая.
Числа пересчитываются программно по заголовкам задач, а не вручную.
...
такой точки 92–97 застынет как обещание, хотя это была прикидка до написания
первой строки кода.
...
ли восемь проверок написаны, а не радоваться опережению.
...
У плана обязательны: точные SHA снимков репозитория и проектных документов, по
которым он написан; цель и наблюдаемый результат; границы и non-goals; входы с
