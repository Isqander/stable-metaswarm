...
план T1.5 остановлен на защищённом вопросе Q44. Оценки не проверены
реализацией. Границы пакетов — §3.1, принятые развилки — §8.
...
`finding_id`/`unchanged_from`, а пустой массив остаётся валидным.
...
reconciliation → правки автора → решение ревьюера с новым observation на
последнем `fix_check` → повторный reconciliation → severity и кап → вопрос
человеку `cap_exhausted_new`. Вход — фикстуры ответов, выход — состояние в
настоящем SQLite.
