...
Шаги 1 и 3–5 из `architecture.md` §4.5: перевод незавершённых попыток в
`interrupted`, поиск осиротевшего процесса по `METASWARM_ATTEMPT_ID` в
`/proc/*/environ`, гашение группы, проверки полноты, перевод веток в
`awaiting_continue`.
...
Шаг 2 из §4.5, отдельной задачей, потому что зависит от T1.12–T1.15: приведение
ветки к `input_sha` прерванной авторской попытки, проверка artifact repo по
digest, дослать неотправленное из outbox.
...
**Зависит от:** T1.12, T1.13, T1.15. Отделено потому, что T1.10a закрывается
раньше и без них, а объявлять recovery готовым до появления reconciler'ов
внешних эффектов было бы неправдой.
...
| **До MVP** | **~88.5–93.5 рабочих дня** |

Первая редакция заявляла 78–83, и это была арифметическая ошибка: сумма задач
фазы 1 составляла 53.5, а не 47. Плюс ревью добавило T1.7b и разделило T1.10 и
T1.22 — ещё 2.5 дня в фазу 1 и 1.5 в фазу 2. Числа выше пересчитаны программно по
заголовкам задач, а не вручную.
