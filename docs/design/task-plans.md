...
последствиями каждой. При создании reviewer attempt до `spawn` одной TX пишет
`reviewer_exposure` по уже resolved provider/model; outcome попытки на это не
влияет и T1.7b exposure задним числом не создаёт.
...
**Дополнительно готово, когда:** попытка с произвольной ревизией отвергается до
`spawn`; артефактный `fix_check` принимает digest ревизии из
