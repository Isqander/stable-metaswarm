...
Дата: 2026-08-03. Статус: декомпозиция фазы 1 готова к пакетному планированию;
оценки не проверены реализацией. Границы пакетов — §3.1, принятые развилки — §8.
...
попытка на шаг не вставляется. Отдельная параметризованная проверка проходит по
всем child-колонкам закрытых enum'ов и подтверждает, что неизвестное значение
отвергается FK.
...
`human`-закрытого, роль `reconciler`.
...
для исправления. Плюс: дословно совпавшие наблюдения склеиваются ключом, разные
...
механическую раскладку по новым ID.
...
override в максимум не входят, а после override накопитель снова растёт.
...
reconciliation → правка автора → решение ревьюера → severity и кап → вопрос
человеку. Вход — фикстуры ответов, выход — состояние в настоящем SQLite.

**Готово, когда:** сценарий проходит от начала до вопроса человеку, и все
значения (`escalation_severity`, счётчики, статус finding'а) читаются из базы
через те же представления, что будет читать рантайм.
...
У плана обязательны: точные baseline SHA кода и артефактов; цель и наблюдаемый
результат; границы и non-goals; входы с потребляемыми результатами; публичные
контракты с единственным владельцем и потребителями; инварианты и отказные
сценарии; изменения по файлам; порядок реализации; атомарные критерии приёмки;
проверки с явным соответствием критериям; риски, допущения и отсутствие
открытых вопросов. Раздел не удаляется, если ответ — «нет»: явное отсутствие
отличимо от забытого анализа.

Критерии приёмки обязаны быть проверяемы без участия автора плана и описывать
поведение: не «реализовано», а «при X происходит Y». Команды проверки пишутся
как `cwd` + argv-массив без shell-синтаксиса, в том же виде, который затем
принимает `verification.plan.v1` (`agent-contracts.md` §7).

План с открытым вопросом не принимается. Если вопрос меняет контракт findings
или правила остановки ветки, работа останавливается до ответа человека; для
остальных публичных границ действует то же правило возврата на ревью из §3.1.
