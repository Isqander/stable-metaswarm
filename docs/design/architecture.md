...
в переход кампании и в той же транзакции выполняет все четыре запроса полноты до
`review_round.result` — включая участие линий, которое проверяется только здесь,
в `discovery`. Так первичный круг
...
|---|---|
| Продолжить деградированным кворумом | `waive_lane_for_round()`: строка `lane_waiver(campaign_id, round_no, lane_id, human_answer_id)`. Круг закрывается по результатам отработавших линий; факт понижения виден в CLI и привязан к ответу человека |
...
Две границы, которых у вариантов выше нет по умолчанию. **Понизить кворум до
нуля нельзя:** waiver можно выписать каждой линии, но круг требует хотя бы
одного успешного мнения, а отказ от проверки — это «остановить ветку», а не
пустой кворум. **Заменить действующего владельца findings нельзя:** в
`fix_check` решения по findings выносит тот, кто их нашёл и помнит (§6.3
`decision.md`), поэтому там доступны только деградация — она findings с
владельца не снимает — и остановка ветки. Расширение вынесено вопросом Q57.
