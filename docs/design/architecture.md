...
синхронный чистый порт `HumanGateFormatter`; только после единственного
`AskHuman` вызывает его внутри writer-транзакции с action и уже собранным
контекстом, проверяет returned envelope и сохраняет human gate. T1.7b владеет
портом и persistence-envelope, T1.16 даёт production-реализацию formatter.
Порт не читает БД, не делает I/O и не принимает собственного решения.
