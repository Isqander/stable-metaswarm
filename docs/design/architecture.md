...
flowchart TB
  ...
  F --> G[запуск задач: ipc, scheduler, liveness, telegram]
  G --> H{есть прогон в работе?}
  H -->|да| I[ждём continue, не продолжаем сами]
  H -->|нет| J[ждём команд]
```

- **Миграции до recovery audit** (`decision.md` §5): иначе audit читает записи
  схемой, которой они не соответствуют.
- **Readiness до recovery**: если бинаря профиля нет, чинить состояние
  бессмысленно; при любом `missing` сервис не стартует и пишет `readiness.json`.
...
```
1. Все step_attempt с outcome IS NULL из прошлых эпох:
     a. по attempt_liveness проверить процесс: pid + proc_start_ticks
     b. группа жива      → TERM → grace → KILL, дождаться
     c. группа мертва    → ничего
     d. outcome = 'interrupted', событие
2. Сверить незавершённые внешние эффекты (§8): checkpoint-коммиты,
   записи в artifact repo, неотправленный outbox.
3. Проверить полноту: наблюдения без связи, круги без результата при
   закрытой кампании, findings без finding_round в открытом круге.
4. Проверить граф: циклы, ложный успех (запрос db-schema §9).
5. Ветки, которые были running → blocked(awaiting_continue).
```

Пункт 1b — тот, ради которого в `attempt_liveness` лежит `proc_start_ticks`:
PID переиспользуется ОС, и «жив ли процесс 4711» без времени старта отвечает про
чужой процесс. Правило `decision.md` §4 буквальное: **новая попытка не
запускается, пока прежняя группа не подтверждена мёртвой.**
...
пересчитывается сопоставлением: после правки автора номера строк сдвигаются, а
модель переописывает ту же проблему другими словами, и контентный ключ ломается
именно во втором круге, ради которого он и нужен.

### 7.2. Вопрос человеку

```
Задать:
  TX: human_question + telegram_outbox + blocker(human_question) + событие
  ── commit ──
  tg_sender: отправка, запись transport_message_id
...
|---|---|---|---|
| Запуск процесса агента | Строка `step_attempt` + `attempt_liveness` | pid, pgid, proc_start_ticks | Группа жива? гасим, потом создаём новую попытку |
| Checkpoint-коммит в клоне | `author_revision` с ожидаемым сообщением | `attempt_id` в сообщении коммита | Ищем коммит с этим `attempt_id`, повторяем только если нет |
...
| Отправка в Telegram | Строка `telegram_outbox` | `question_id`, `update_id` на входящих | Неотправленное в outbox → отправить; дубль входящего → PK |

Без идентификатора в сообщении коммита падение между `git commit` и фиксацией
состояния даёт второй коммит с тем же содержимым. Это не гипотетика: именно эта
граница проверяется приёмочным сценарием «убийство процесса после завершения до
фиксации состояния».

Гарантия по Telegram называется честно: **durable at-least-once с идемпотентной
обработкой**, а не exactly-once, которого у внешнего эффекта не бывает.
...
потому единственное, где четыре запрета проверяются кодом, а не промптом.
