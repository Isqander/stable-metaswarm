...
в переход кампании и в той же транзакции выполняет все три запроса полноты до
`review_round.result`. Так первичный круг
...
4. Цикл правок
   loop:
     ...
     ├─ assert: у каждого слота кворума есть успешная попытка ревьюера
     │          этого круга либо lane_waiver на этот круг
...
|---|---|
| Продолжить деградированным кворумом | `waive_lane_for_round()`: строка `lane_waiver(campaign_id, round_no, lane_id, human_answer_id)` + событие. Круг закрывается по результатам отработавших линий; факт понижения виден в CLI и привязан к ответу человека |
| Заменить линию другим профилем | `replace_lane_assignment()`: новое поколение `lane_assignment` в том же слоте + событие. Отбор идёт заново, с теми же правилами свежести и запретом на автора |
| Остановить ветку | Ветка неуспешна, соседние продолжают |

**Кворум — это слоты, а не исполнители.** Замена не меняет ни их число, ни
владение findings: `finding_round.owner_lane_id` ссылается на слот, поэтому
решения владельца после замены выносит новое поколение той же линии. Обе
операции описаны в `db-schema.md` §5.2 вместе с гейтом, который не даёт закрыть
круг, пока каждый слот не отработал или не отпущен waiver'ом.
