...
**Повтор прерванной авторской попытки начинается от `input_sha`.** Искать
коммит с `attempt_id` и «не повторять, если он есть» — плохая идея: коммит
прерванной попытки фиксирует незаконченную работу, и продолжать поверх него
значит строить на середине чужой мысли. Ветка жёстко приводится к состоянию до
попытки; работа прерванного агента остаётся достижимой через reflog и в
транскрипте, но в линии работы её нет. Идентификатор в сообщении коммита при
этом сохраняется — для аудита и для того, чтобы такие коммиты были видны как
принадлежащие прерванной попытке.
