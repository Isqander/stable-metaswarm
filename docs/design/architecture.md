...
в одном human gate и одно действие для кампании в ответе. Production-
реализацией не проверена.
...
**В очередь передаётся снимок, а не ссылка на mutable input.** Проверенный
ответ агента глубоко frozen: вложенные JSON-массивы уже tuple, nested models и
policy rejections неизменяемы. `NewRunEvent` при создании рекурсивно проверяет
JSON payload, включая UTF-8-кодируемость каждой строки/ключа, сразу кодирует
каноническую JSON-строку и хранит только её, а не живое дерево caller'а. Поэтому
изменение исходного списка/словаря после enqueue не меняет то, что увидит
writer, а попытка менять сам `ValidatedAgentResult` отвергается до начала
транзакции.
...
2. Проверить полноту тремя нормативными запросами `db-schema.md` §5.2/§5.4:
   наблюдения без связи; незавершённые `issued`-строки `finding_round`;
   открытые findings кампании без строки участия в текущем круге. Отдельно
   найти круги с решениями, но без результата. `post_check` законен в
   закрываемом круге:
...
4. Проверить пары branch-scoped blocker ↔ `branch.state`: открытый
   ...
   автоматически.
5. Оставшиеся после п. 4 ветки, которые были `running`, перевести в
   `blocked(awaiting_continue)`.
```

Пункт 5 — не мелочь. Ветка, которая была в работе, после рестарта не готова к
...
в переход кампании и в той же транзакции выполняет все четыре гейта полноты до
`review_round.result` — включая участие линий, которое проверяется только здесь,
в `discovery`, и состоит из трёх запросов: слоты без работы текущего
исполнителя, круг без единого мнения и состав слотов, не равный заявленному
кворуму. Гейтов четыре, запросов шесть. Так первичный круг
...
Форма вопроса не готовится вызывающим заранее: тогда ему пришлось бы повторить
`decide_after_check`, чтобы угадать reason. `ReviewTransactions` получает
синхронный чистый порт `ReviewDecisionGateFormatter`; только после единственного
`AskHuman` заворачивает его в `AfterCheckGate` и вызывает порт внутри
writer-транзакции с этой обёрткой и уже собранным контекстом, после чего
сохраняет human gate. Два других варианта того же union'а — пути Q49 (§7.1.2).
Порт возвращает только представление; `reason`, scope, snapshot и транспорт
вопроса транзакция берёт из самого запроса и конфигурации, а не из ответа порта
— в P1-A транспорт всегда `cli`. T1.7b владеет портом и
persistence, T1.16 даёт production-реализацию. Порт не читает БД, не делает I/O
и не принимает собственного решения.
...
1. Открытие кампании — T1.18 `open_review_campaign()`
   ...
   └─ TX: campaign + слоты review_lane + первые lane_assignment
          + round(1, discovery) + событие
...
2а. Слепая фаза завершена — T1.7b `complete_discovery()`
   ...
   └─ TX: campaign.state='reconciliation' + событие discovery_completed,
      ...
      всё ещё видит discovery и не отличает «ждём вторую линию» от
      «ждём reconciliation»

3. Первичный reconciliation
   ├─ вход: наблюдения всех линий + ledger (открытые ID + закрытые
   │  из кампаний, чей subject входит в текущий)
   ├─ каждое raw observation передаётся отдельной позицией со своим ID/evidence;
   │  dedup_key остаётся диагностикой и не влияет на вход или identity (Q56)
...
4. Цикл правок
   loop:
     ...
     ├─ assert: каждый открытый finding, уже вошедший в campaign,
     │          имеет строку участия в текущем круге
...
|---|---|
| Продолжить деградированным кворумом | `waive_lane_for_round()`: строка `lane_waiver(campaign_id, round_no, lane_id, human_answer_id)`. Круг закрывается по результатам отработавших линий; факт понижения виден в CLI и привязан к ответу человека. **Только в `discovery`** |
| Заменить линию другим профилем | `replace_lane_assignment()`: новое поколение `lane_assignment` в том же слоте. Отбор идёт заново, с теми же правилами свежести и запретом на автора. **Только в `discovery`** |
| Остановить ветку | Ветка неуспешна, соседние продолжают |

**Кворум — это слоты, а не исполнители,** поэтому замена не меняет их число.
Обе операции — не самостоятельные транзакции, а review-следствие приёма ответа
(§6.6, «Получен ответ»): иначе между снятой блокировкой и применённым действием
...
- деградация бессмысленна: пропустить можно только владельца открытых findings,
  а его строки `issued` всё равно требуют решения, и круг не закроется. Принять
  ответ и записать durable no-op хуже, чем не предлагать вариант.

Ещё одна граница, которой у вариантов нет по умолчанию: **понизить кворум до
нуля нельзя.** Waiver можно выписать каждой линии, но круг требует хотя бы
одного успешного мнения — отказ от проверки это «остановить ветку», а не пустой
кворум.

Гейты, которые всё это проверяют, — в `db-schema.md` §5.2: участие линий
(только `discovery`) и покрытие findings (оба вида круга).
...
Для review-вопросов `dispute` и `cap_exhausted_*` свободный текст сначала
сводится к одному из трёх кампанийных действий. Смешанная per-finding раскладка
не доходит до транзакции `human_answer`: вопрос остаётся открыт, увеличивается
`reask_count` и через outbox уходит переспрос. После лимита принимается только
ключ варианта. Поэтому показанный ниже путь «Ответить» начинается уже после
успешной интерпретации и никогда не применяет часть ответа.
...
В `verification.plan.v1` каждый шаг ожидает только `expect = exit_zero`.
Поддержка ожидаемого ненулевого кода, текста в output или иной семантики — это
новая версия схемы: валидатор и исполнитель не расширяют множество независимо.
