...
JSON payload, сразу кодирует каноническую JSON-строку и хранит только её, а не
живое дерево caller'а. Поэтому изменение исходного списка/словаря после enqueue
не меняет то, что увидит writer, а попытка менять сам `ValidatedAgentResult`
отвергается до начала транзакции.
